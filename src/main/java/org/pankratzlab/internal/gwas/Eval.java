...
public class Eval {
	...
	public Eval(LinkedHashMap<String, String> evalArgs, ArrayList<String> quantFactorNames, Path results, int iterations) {
	...
	public void evaluateFinalResults() {
		try (BufferedReader resultsReader = Files.getAppropriateReader(results.toString())){
			PrintWriter writer = Files.getAppropriateWriter(results.getRoot().toString() + "evalResults.txt");
