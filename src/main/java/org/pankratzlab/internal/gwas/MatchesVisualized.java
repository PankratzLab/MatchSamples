...
public class MatchesVisualized {
  ...
  public MatchesVisualized(String dir, String samplesFile, String factorfile, int[] factorIndices,
    ...
    for (int i = 0; i < controls.length; i++) {
      data[cases.length + i] = ArrayUtils.toDoubleArray(hash.get(controls[i])
    ...
    if (v.size() != cases.length) {
      System.err.println("Error - number of pairings (" + v.size()
                         + ") doesn't match number of anchors loaded (" + cases.length + ")");
      System.exit(1);
    }
      ...
      public void paintComponent(Graphics g) {
          ...
          if (!Double.isNaN(dists[i])) {
            Grafik.drawThickLine(g,
                                 ...
                                 getHeight() - (int) (data[pairs[i][0]][y]
                                                    ...
                                                    - HEIGHT_BUFFER,
                                 (int) (data[cases.length + pairs[i][1]][x]
                                        * (getWidth() - 2 * WIDTH_BUFFER)) + WIDTH_BUFFER,
                                 getHeight() - (int) (data[cases.length
                                                           + pairs[i][1]][y]
                                                      * (getHeight() - 2 * HEIGHT_BUFFER))
                                                                                           - HEIGHT_BUFFER,
        ...
        for (int i = 0; i < cases.length; i++) {
          g.fillOval((int) (data[i][x] * (getWidth() - 2 * WIDTH_BUFFER)) + WIDTH_BUFFER - SIZE / 2,
                     getHeight() - (int) (data[i][y] * (getHeight()
                                                        - 2 * HEIGHT_BUFFER)) - HEIGHT_BUFFER - SIZE
                                                                                                / 2,
                     SIZE, SIZE);
        }
        ...
        if (!hideExtraControls) {
          for (int i = cases.length; i < data.length; i++) {
            g.fillOval((int) (data[i][x] * (getWidth() - 2 * WIDTH_BUFFER)) + WIDTH_BUFFER
                       - SIZE / 2,
