...
public class MatchSamples {
	...
	public static String matchPairs(String dir, String distanceFile, boolean minMin_not_maxMin, int iterations) {
		return matchPairs(dir, distanceFile, minMin_not_maxMin, false, iterations);
	}

	/**
	 * @param dir               directory with file
	 * @param distanceFile      distance file , computed from
	 *                          {@link MatchSamples#matchMaker}
	 * @param minMin_not_maxMin this is a confusing argument
	 * @param antiOptimal       instead of selecting the optimal match, do the
	 *                          opposite. You most likely want this to be false
	 * @return
	 */
	public static String matchPairs(String dir, String distanceFile, boolean minMin_not_maxMin, boolean antiOptimal,
			int iterations) {
		BufferedReader reader;
		PrintWriter writer;
		...
		try {
					...
					for (int i = 0; i < cases.length; i++) {
						if (antiOptimal) {
							mins[i] = (matches[i][iter] == -1 ? findMinOrMaxIndex(dists[i], usedControls, false)
									: (minMin_not_maxMin ? Double.NEGATIVE_INFINITY : Double.NEGATIVE_INFINITY));
						} else {
			...
			writer = Files.openAppropriateWriter(
		...
		System.out.println("Created " + ext.rootOf(distanceFile) + "_" + (minMin_not_maxMin ? "min" : "max") + "Min.xln"
				+ " in " + ext.getTimeElapsed(time));

		return ext.rootOf(distanceFile) + "_" + (minMin_not_maxMin ? "min" : "max") + "Min.xln";
	}

	public static double findMinOrMaxIndex(double[] distances, Set<Integer> skipIndices, boolean findMin) {
		int index = 0;
		double cur;
		if (findMin) {
	...
	public static void evalAgeSex_and_MDS_separately(String dir, String pairings, String refDistances, String demofile,
			String ageHead, String genHead) {
		BufferedReader reader;
		PrintWriter writer;
		...
		try {
			...
			while (reader.ready()) {
				...
				if (ext.indexOfStr(line[0], anchors) >= 0) {
					...
					data[ext.indexOfStr(line[0], barnacles)][1] = new int[] { Integer.parseInt(line[ageIndex]),
							Integer.parseInt(line[genIndex]) };
				}
			}
			reader.close();
		...
		results = "Cases: " + ext.formPercent(sumGenders[0], 0) + " male, mean AOO=" + ext.formDeci(sumAges[0], 1, true)
				+ "\n" + "Controls: " + ext.formPercent(sumGenders[1], 0) + " male, mean AOO="
		...
		results += "Cases: " + ext.formPercent(sumGenders[0], 0) + " male, mean AOO="
				+ ext.formDeci(sumAges[0], 1, true) + "\n" + "Controls: " + ext.formPercent(sumGenders[1], 0)
				+ " male, mean AOO=" + ext.formDeci(sumAges[1], 1, true) + "\n" + ext.formPercent(sumGenders[2], 0)
				+ " gender concordance\n" + "mean age diff b/w case-ctrl: " + ext.formDeci(sumAges[2], 1, true) + "\n"
				+ "Mean MDS distance: " + ext.formDeci(ArrayUtils.mean(mdsDists), 2) + " (SD: "
				+ ext.formDeci(ArrayUtils.stdev(mdsDists), 3, true) + ", range="
				+ ext.formDeci(ArrayUtils.min(mdsDists), 2, true) + "-"
				+ ext.formDeci(ArrayUtils.max(mdsDists), 2, true) + ")\n";
	...
	public static void eval(String dir, String pairings, String demofile, String[] factorVars) {
		...
		try {
			...
			for (int i = 0; i < factors.length; i++) {
				...
				if (indices[i] == -1) {
					System.err.println("Error - could not find factor '" + factors[i] + "' in demographics file '"
							+ demofile + "'");
					problem = true;
				}
			}
		...
		for (int outliers = 0; outliers < 2; outliers++) {
			...
			for (int i = 0; i < factors.length; i++) {
				...
				if (checkForConcordance[i]) {
					...
					results += "\t" + ext.formDeci(sums[i][0], 1, true) + "\t" + ext.formDeci(sums[i][1], 2, true)
			...
			results += "Mean distance: " + ext.formDeci(ArrayUtils.mean(dists), 2) + " (SD: "
					...
					+ ext.formDeci(ArrayUtils.min(dists), 2, true) + "-" + ext.formDeci(ArrayUtils.max(dists), 2, true)
					+ ")\n";
		}
	...
	public static void matchFromParameters(String filename, Logger log) {
		...
		if (paramV != null) {
			...
			/*
			 * try { file = matchMaker("", casesFile, controlsFile, factorFile, factors,
			 * factorWeights, normalize); file = normalizeDistances("", file, 0, 100); pairs
			 * = matchPairs("", file, true); eval("", pairs, demographicsFile, demoFactors);
			 * new MatchesVisualized("", casesFile, controlsFile, coordsFile, coords,
			 * pairs); pairs = matchPairs("", file, false); new MatchesVisualized("",
			 * casesFile, controlsFile, coordsFile, coords, pairs); eval("", pairs,
			 * demographicsFile, demoFactors); new MatchesVisualized("", casesFile,
			 * controlsFile, coordsFile, coords, pairs); ext.waitForResponse(); } catch
			 * (Exception e) { log.reportError("Error matching files");
	...
	public static String buildVisFile(String dir, String fullResultsFile, int currentIteration) {
		...
		PrintWriter writer;
		int controlColumn = (currentIteration == 0 ? (currentIteration + 1) : ((currentIteration * 2) + 1));
		int distanceColumn = controlColumn + 1;
		Vector<String> data = HashVec.loadFileToVec(dir + "/matches/" + fullResultsFile, true, false, true);
	...
	public String run(int iterations, Logger log) {
		...
		try {
			 ...
			 * "/../" + factorsFile, "AGE01", "SEX"); MatchSamples.eval(matchesDirectory,
	...
	public static void main(String[] args) {
		...
		if (visOnly) {
			...
			if (factorNames.length < 2) {
				System.err.println("At least two factors are required. Found: " + ArrayUtils.toStr(factorNames, ","));
				System.exit(1);
			}
